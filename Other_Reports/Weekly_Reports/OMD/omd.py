...
def omd_initial(FILE):

    date_range = ui.date_picker(label='Select Date Range', mode='range', key='omd_date_range', default_value=None)

    if date_range != None:

        st.button(label='Submit', key='submit_omd_data')
        
        if st.session_state['submit_omd_data']:
